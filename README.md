...
schedule. The app intentionally keeps two demo paths: a bundled seed result
for first paint and synthetic fallback data when live DAM data is unavailable.

## Architecture

```
backend/                FastAPI pipeline (Modules 1–4)
  ingest.py             Module 1 — Greek DAM + weather ingestion, SQLite
  forecast.py           Module 2 — sklearn HGB quantile or ridge fallback
...
`/seed_optimization.json`. Click **Run Optimization** to call the backend and
replace the seed output with a freshly computed run.
...
3. Add environment variable `ENTSOE_API_KEY` (optional — without it the
   Greek DAM ingestion falls back to flagged synthetic data).
...
| ----------------------- | --------- | ----------------------------------------------- |
| `ENTSOE_API_KEY`        | backend   | Optional live Greek DAM price ingestion         |

## Notes on data scarcity

Because Greek standalone batteries only entered the DAM in test mode in April
2026, there is essentially no battery telemetry to learn from. The framework
therefore:

* relies on **market signals** rather than battery telemetry;
...
* trains the current price model on **Greek DAM history joined with Athens
  weather features**;
* keeps **reference-market weather feeds** in the ingestion layer and UI for
  context/provenance, but does not currently fit a direct cross-market price
  model;
